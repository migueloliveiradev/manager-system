...
export const useAuth = () => {
  ...
  const login = async (email: string, password: string) => {
    const response = await $fetch<BaseResponse<AuthResponse>>('/api/auth/login', { method: 'POST', body: { email, password } })
  ...
  const register = async (fullName: string, email: string, password: string, role = 'Manager') => {
    const response = await $fetch<BaseResponse<AuthResponse>>('/api/auth/register', { method: 'POST', body: { fullName, email, password, role } })
    if (response.hasErrors || !response.data) throw new Error(response.errors.join(', '))
    token.value = response.data.accessToken
    refreshToken.value = response.data.refreshToken
  }
